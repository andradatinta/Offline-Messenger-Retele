...
int loginUser(char * username, MYSQL * con)
{
    ...
    MYSQL_ROW row; // variabila folosita pentru parcurgerea randurilor din rezultat
    int num_fields = mysql_num_fields(query_result);

    while ((row = mysql_fetch_row(query_result))) {
        //Asta e o verificare de siguranta, in mod normal daca ajungi sa fii in while-ul asta implicit query-ul a dat macar un rand, adica ai putea face din prima
        //userExists = 1; break; (ca sa iesi din while ca nu mai ai treaba), dar mai bine sa fie totusi
      ...
      printf("inainte de if-ul in care compar row[1] cu username, userExists este %d \n", userExists);
        // printf("intri in if?: %s \n", row[1]);

        // if (strcmp(row[1], username) == 0) {
            
        //     //Trebuie facuta comparatia intre prima coloana returnata de query si username-ul tau, eu sunt in visual studio si nu mi apar live erorile in terminal
        //     //posibil sa mai ai nevoie de conversii de tipuri ca sa fie ambele char*/char[], vezi tu exact
        //     userExists = 1;
        //     break;
        // }
        userExists = 1;
...
void modifyStatus(char * username, MYSQL * con )
{
    ...
    // if(result == NULL)
    // { 
    //     finish_with_error(con); // nu prea vreau asta ca imi inchide aiurea procesul 
    // }
...
int signupUser( char * username, MYSQL * con)
{
    ...
    if (userExists == 0)
    {
        ...
        strcat(query2, end);
        printf("interogarea pt signup: %s \n", query2);

        mysql_query(con, query2);
        
    }

    printf("userexists in signup: %d \n", userExists);
    return userExists;
}
...
{   int id;
    ...
    char * query2;
    query2 = malloc(strlen(query1) + strlen(username) + 2);
    printf("username din getUserid: %s \n", username);
    strcpy(query2, query1);
    strcat(query2, username);
    deleteNewline(query2);
    strcat(query2, end);
    printf("interogarea pt getUserid: %s \n", query2);
...
int sendMessage(int convo_id, int client, char * sender, MYSQL * con)
{
    ...
    char convo_ids[5];
    int convo=0;
    // int maxBytes = sizeof(message);
    // readMessage(message, client, sizeof(message)); // stocheaza mesajul scris de client in message 
    int len = read(client, message, 255);
    message[len] = '\0';
    printf("len: %d \n", len);
    
    // receiveCommand(message, client, maxBytes);
    ...
    {
        char query1_insert[255] = "insert into messages (convo_id, message, sender_id) values (";
        char comma[2]=",";
        char sep_mess[2]="'";
        char end_insert[10] = ");";
        char * query2_insert;
        ...
        {
            sendMessage(convo_id, client, sender, con);
        }
...
//   mysql_free_result(result);
  ...
  if(convo_id == -1)
  {
    // char * failedhis = malloc(strlen(failed));
    // strcpy(failedhis, failed);
    // return failedhis;
...
if(sendMessage(convo_id, client, user_id, con) == -1)
        return convo_id;
...
int readUsername (char * comm, int client, size_t maxBytes)
{
    int check = -1;
    bzero(comm, maxBytes);
    if(read(client, comm, maxBytes) != -1)
        check=1;
    return check; // daca o sa am ce citi, o sa mai apelez o data processCommand cu username-ul asta
    // ar trebui sa modific mesajele transmise din server in client basically cam peste tot, mereu sa am username-ul in
...
void processCommand (char * answer, char * comm, int client, MYSQL * con, int fd[2])
{
    ...
    {
        printf("intru si in if \n ");
        if(userExists != 1)
        {
            printf("merge cu userExists \n ");
            bzero(answer, sizeof(answer));
